...
(function () {
    'use strict';
    console.log('[SD Helper v31] Script loaded (Direct Function Call).');

    const SCRIPT_ID = 'sd_gen_standard_v31';
    const STORAGE_KEY = 'sd_gen_settings_v31';
    ...
    async function fetchModels(baseUrl, apiKey) {
        try {
            ...
            console.error(e);
            throw new Error(e.message || '连接失败，请检查URL或API Key');
        }
    }

    async function callLLMForUpdate(prompt, instruction) {
        const config = settings.llmConfig;
        if (!config.baseUrl) throw new Error("API URL 未配置");
    ...
    function injectGlobalStyles() {
        if ($('#sd-global-css-v31').length) return;
        $('<style id="sd-global-css-v31">').text(GLOBAL_CSS).appendTo('head');
    ...
    function updateWrapperView($wrap, images, idx) {
        ...
        if (count === 0) {
            ...
            $left.toggle(idx > 0);
            $del.show();
            if (idx === count - 1) $right.addClass('gen-mode').attr('title', '生成新图');
            else $right.removeClass('gen-mode').attr('title', '下一张');
    ...
    function processChatDOM() {
        ...
        $('.mes_text').each(function() {
            ...
            if (hasTHRender) {
                console.log('[SD] 检测到TH-render，使用精准替换模式');
                ...
                $el.children().each(function() {
                    ...
                    if (childHtml && childHtml.indexOf(sTag) !== -1) {
                        ...
                        if (hasChange) {
                            $child.html(newHtml);
                            processedCount++;
                            console.log('[SD] 已处理子元素:', $child[0].tagName, '块索引:', blockIdx - 1);
                ...
                if (processedCount > 0) {
                    console.log(`[SD] 精准替换完成，处理了 ${processedCount} 个元素，共 ${blockIdx} 个块`);
                }
    ...
    function createUIHtml(prompt, images, preventAuto, blockIdx, initIdx) {
        ...
        <div class="sd-ui-container">
            <div class="sd-ui-wrap" 
                ...
                <div class="sd-ui-toggle" title="折叠/展开">▵</div>
                <div class="sd-ui-viewport">
                    <div class="sd-zone top" title="编辑提示词"></div>
                    <div class="sd-zone left" title="上一张" style="display:${initIdx > 0 ? 'block' : 'none'}"></div>
                    <div class="sd-zone right ${hasImages && initIdx === images.length-1 ? 'gen-mode' : ''} ${!hasImages ? 'gen-mode' : ''}" 
                         title="${hasImages && initIdx < images.length-1 ? '下一张' : '生成新图'}"></div>
                    <div class="sd-zone delete" title="删除当前图" style="display:${hasImages ? 'block' : 'none'}"></div>
                    ...
                    <div class="sd-placeholder" style="display:${hasImages ? 'none' : 'block'}">
                        <i class="fa-solid fa-image"></i> 等待生成...<br>
                        <small>（索引: ${blockIdx}）</small>
                    </div>
                </div>
            </div>
        </div>`;
    ...
    function addMenuItem() {
        ...
        const menuItemHTML = `
            <div class="list-group-item flex-container flexGap5 interactable" id="${menuItemId}" title="设置SD生图助手">
        ...
        const html = `
            <div style="padding:10px;">
                ...
                <div id="${aiBoxId}" class="sd-ai-update-box">
                    <label style="font-size:0.9em; font-weight:bold;">AI修改指令 (Instruction):</label>
                    <textarea id="${aiInputId}" class="text_pole" rows="2" placeholder="例如：把衣服改成红色，增加下雨背景..." style="width:100%; margin:5px 0;"></textarea>
                    <div style="text-align:center; margin-top:5px;">
                        <button id="${btnAiRunId}" class="menu_button" style="font-size:0.9em; width:100%;">🚀 执行更新</button>
                    </div>
                </div>

                <div style="display:flex; gap:10px; margin-top:10px;">
                    <button id="${btnAiToggleId}" class="menu_button" style="flex:1;" title="AI 辅助修改">🪄 AI优化</button>
                    <button id="${btnModId}" class="menu_button" style="flex:1;">仅修改</button>
                    <button id="${btnGenId}" class="menu_button" style="flex:1; background:var(--SmartThemeQuoteColor); color:#fff;">生成</button>
                </div>
            </div>`;
    ...
    function openSettingsPopup() {
        ...
        const popupHtml = `
            <div style="padding: 10px; max-height: 70vh; overflow-y: auto; overflow-x: hidden; scrollbar-width: thin;">
                <h3 style="margin-bottom: 15px;">🎨 SD生图助手 v31</h3>
                <div class="sd-tab-nav">
                    <div id="${tabInjId}" class="sd-tab-btn active">注入(Prompt)</div>
                    <div id="${tabCfgId}" class="sd-tab-btn">基础设置</div>
                    <div id="${tabApiId}" class="sd-tab-btn">API设置</div>
                    <div id="${tabLogId}" class="sd-tab-btn">日志</div>
                </div>
                <div id="${contentInjId}" class="sd-tab-content active">
                    <div style="margin-bottom:15px; padding:10px; background:rgba(0,0,0,0.1); border-radius:5px;">
                        <label class="checkbox_label" style="display: flex; align-items: center; margin-bottom:10px;">
                            ...
                            <span style="margin-left: 8px; font-weight:bold;">启用提示词注入 (Context Injection)</span>
                        </label>
                        <label class="checkbox_label" style="display: flex; align-items: center; margin-bottom:10px;">
                            <input type="checkbox" id="sd-ai-tmpl-enable" ${settings.aiUpdateWithTemplate ? 'checked' : ''} /> 
                            <span style="margin-left: 8px;">AI修图时携带完整模版 (Include Template in AI Update)</span>
                        </label>
                        <div style="display:flex; gap:15px; margin-top:10px;">
                            <div style="flex:1"><label>插入深度 (0=最后):</label><input type="number" id="sd-inject-depth" class="text_pole" value="${settings.injectDepth}" min="0" max="50" style="width:100%" /></div>
                            <div style="flex:1">
                                <label>发送身份 (Role):</label>
                                <select id="sd-inject-role" class="text_pole" style="width:100%">
                                    <option value="system" ${settings.injectRole === 'system' ? 'selected' : ''}>System (系统)</option>
                                    <option value="user" ${settings.injectRole === 'user' ? 'selected' : ''}>User (用户)</option>
                                    <option value="assistant" ${settings.injectRole === 'assistant' ? 'selected' : ''}>Assistant (AI)</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div style="margin-bottom:15px; text-align:right;"><small style="color:#aaa; cursor:pointer;" id="sd-mode-toggle">[切换模式: ${settings.useRawMode ? '原始文本' : '角色构造器'}]</small></div>
                    <div id="sd-mode-builder" style="display:${settings.useRawMode ? 'none' : 'block'}">
                        <h4 style="margin-bottom:10px; border-bottom:1px solid #555; padding-bottom:5px;">角色特征数据库 (Character DB)</h4>
                        <div id="sd-char-list-container"></div>
                        <button id="sd-add-char" class="sd-add-btn">+ 添加角色 (Add Character)</button>
                        <div class="sd-accordion" id="sd-tpl-top-header"><span>前置模版 (Template Top)</span> <span>▿</span></div>
                        ...
                        <div class="sd-accordion" id="sd-tpl-bot-header"><span>后置模版 (Template Bottom)</span> <span>▿</span></div>
                ...
                <div id="${contentCfgId}" class="sd-tab-content">
                    <label class="checkbox_label" style="margin-bottom: 15px; display: flex; align-items: center;"><input type="checkbox" id="sd-enable-toggle" ${settings.enabled ? 'checked' : ''} /> <span style="margin-left: 8px;">启用生图解析功能 (Image Parsing)</span></label>
                    <div style="margin-bottom: 10px;"><label>全局前缀 (Prefix):</label><textarea id="sd-prefix" class="text_pole" rows="2" style="width:100%; font-family:monospace">${settings.globalPrefix}</textarea></div>
                    <div style="margin-bottom: 10px;"><label>全局后缀 (Suffix):</label><textarea id="sd-suffix" class="text_pole" rows="2" style="width:100%; font-family:monospace">${settings.globalSuffix}</textarea></div>
                    <div style="margin-bottom: 15px;"><label>全局负面 (Negative):</label><textarea id="sd-negative" class="text_pole" rows="3" style="width:100%; font-family:monospace">${settings.globalNegative}</textarea></div>
                    <div style="margin-top:20px; border-top:1px solid #555; padding-top:15px;">
                        <label style="font-weight:bold;">配置同步 (Configuration Sync):</label>
                        <div style="display:flex; gap:10px; margin-top:10px;">
                            <button id="${btnExpId}" class="menu_button" style="flex:1;">📤 导出配置 (Export)</button>
                            <button id="${btnImpId}" class="menu_button" style="flex:1;">📥 导入配置 (Import)</button>
                        </div>
                    </div>
                </div>
                <div id="${contentApiId}" class="sd-tab-content">
                    <div class="sd-placeholder" style="margin-bottom:15px; text-align:left; padding:10px;">在此配置兼容 OpenAI 格式的 API以支持"AI更新"功能。</div>
                    <div style="margin-bottom: 10px;"><label>API基础URL (Base URL):</label><input type="text" id="sd-api-url" class="text_pole" style="width:100%" value="${settings.llmConfig.baseUrl}" placeholder="http://127.0.0.1:7861/v1" /></div>
                    <div style="margin-bottom: 10px;"><label>API密钥 (API Key):</label><input type="password" id="sd-api-key" class="text_pole" style="width:100%" value="${settings.llmConfig.apiKey}" placeholder="sk-..." /></div>
                    <div style="display:flex; gap:10px; margin-bottom:10px; align-items:flex-end;">
                        <div style="flex:1"><label>选择模型:</label><select id="sd-api-model" class="text_pole" style="width:100%"><option value="${settings.llmConfig.model}">${settings.llmConfig.model || '未选择'}</option></select></div>
                        <button id="sd-api-load-btn" class="menu_button" style="width:100px; height:38px;">连接加载</button>
                    </div>
                    <div style="display:flex; gap:10px;">
                         <div style="flex:1"><label>最大Tokens (生成长度):</label><input type="number" id="sd-api-tokens" class="text_pole" value="${settings.llmConfig.maxTokens}" style="width:100%" placeholder="4096" /></div>
                        <div style="flex:1"><label>温度 (Temperature):</label><input type="number" id="sd-api-temp" class="text_pole" value="${settings.llmConfig.temperature}" step="0.1" max="2.0" style="width:100%" /></div>
                    </div>
                </div>
        ...
        setTimeout(() => {
            ...
            const renderCharRow = (name, tags, enabled) => {
                const $row = $(`<div class="sd-char-row"><input type="checkbox" class="sd-char-enable" ${enabled?'checked':''} style="margin-top:10px; margin-right:5px; cursor:pointer;" title="是否启用此角色" /><input type="text" class="text_pole sd-char-name" placeholder="角色名" value="${name}" /><textarea class="text_pole sd-char-tags" rows="2" placeholder="固定特征 (Fixed Tags)">${tags}</textarea><button class="sd-char-del" title="删除">✕</button></div>`);
            ...
            const toggleAccord = (headerId, contentId) => {
                $(`#${headerId}`).on('click', () => {
                    ...
                    $(`#${headerId} span:last-child`).text($(`#${contentId}`).hasClass('show') ? '▵' : '▿');
                });
            };
            ...
            $('#sd-mode-toggle').on('click', () => {
                ...
                $('#sd-mode-toggle').text(`[切换模式: ${settings.useRawMode ? '原始文本' : '角色构造器'}]`);
            });

            $(`#${btnExpId}`).on('click', function() {
                 const html = `<div style="padding:10px;"><h3>导出配置 (Export)</h3><textarea class="text_pole" rows="10" style="width:100%; font-family:monospace; font-size:0.8em;">${JSON.stringify(settings, null, 2)}</textarea></div>`;
                 SillyTavern.callGenericPopup(html, 1, '', { wide: false });
            });
            $(`#${btnImpId}`).on('click', function() {
                 const taId = `sd-imp-ta-${Date.now()}`; const confirmId = `sd-imp-confirm-${Date.now()}`;
                 const html = `<div style="padding:10px;"><h3>导入配置 (Import)</h3><textarea id="${taId}" class="text_pole" rows="10" style="width:100%; font-family:monospace; font-size:0.8em;"></textarea><button id="${confirmId}" class="menu_button" style="width:100%; margin-top:10px; background:var(--SmartThemeQuoteColor); color:#fff;">确认导入</button></div>`;
                 SillyTavern.callGenericPopup(html, 1, '', { wide: false });
                 setTimeout(() => {
                     $(`#${confirmId}`).on('click', () => {
                         try { settings = { ...DEFAULT_SETTINGS, ...JSON.parse($(`#${taId}`).val()) }; saveSettings(); toastr.success('配置导入成功！'); closeCurrentPopup(this); } 
                         catch(e) { toastr.error('解析错误: ' + e.message); }
                     });
                 }, 100);
            });
            ...
            $('#sd-api-load-btn').on('click', async function() {
                 ...
                 if(!url) { toastr.warning('请输入基础URL'); return; }
                 ...
                 } catch(e) { toastr.error('加载失败: ' + e.message); }
                 finally { btn.prop('disabled', false).text('连接加载'); }
            });
            ...
            $(`#${btnSaveId}`).on('click', function() {
                ...
                saveSettings(); toastr.success('设置已保存'); closeCurrentPopup(this); processChatDOM();
            });

            $(`#${btnResetId}`).on('click', function () {
                if (!confirm('确定要重置为默认设置吗？这会覆盖当前所有配置。')) return;
                ...
                if (typeof toastr !== 'undefined') {
                    toastr.success('已重置为默认设置，请关闭并重新打开设置窗口查看。');
                } else {
                    alert('已重置为默认设置，请关闭并重新打开设置窗口查看。');
                }
    ...
    function registerSTEvents() {
        ...
        try { 
            ...
            addLog('EVENT', '✅ 已注册事件: chat_completion_prompt_ready');
        } catch(e) {
        ...
        if (eventTypes.USER_MESSAGE_RENDERED || 'user_message_rendered') {
            ...
            SillyTavern.eventSource.on(eventName, (messageId) => {
                addLog('EVENT', `✅ [USER_MESSAGE_RENDERED] 触发 - 消息ID: ${messageId}`);
                debouncedProcess();
            });
        ...
        if (eventTypes.CHARACTER_MESSAGE_RENDERED || 'character_message_rendered') {
            ...
            SillyTavern.eventSource.on(eventName, (messageId) => {
                addLog('EVENT', `✅ [CHARACTER_MESSAGE_RENDERED] 触发 - 消息ID: ${messageId}`);
                debouncedProcess();
            });
            addLog('EVENT', `✅ 已注册事件: ${eventName}`);
        }
        ...
        if (eventTypes.CHAT_CHANGED || 'chat_changed') {
            ...
            SillyTavern.eventSource.on(eventName, (chatFilename) => {
                addLog('EVENT', `✅ [CHAT_CHANGED] 触发 - 聊天文件: ${chatFilename}`);
                debouncedProcess();
            });
            addLog('EVENT', `✅ 已注册事件: ${eventName}`);
        }
        ...
        if (eventTypes.MESSAGE_UPDATED || 'message_updated') {
            ...
            SillyTavern.eventSource.on(eventName, (messageId) => {
                addLog('EVENT', `✅ [MESSAGE_UPDATED] 触发 - 消息ID: ${messageId}`);
                debouncedProcess();
            });
            addLog('EVENT', `✅ 已注册事件: ${eventName}`);
        }
        ...
        if (eventTypes.MESSAGE_SWIPED || 'message_swiped') {
            ...
            SillyTavern.eventSource.on(eventName, (messageId) => {
                addLog('EVENT', `✅ [MESSAGE_SWIPED] 触发 - 消息ID: ${messageId}`);
                debouncedProcess();
            });
            addLog('EVENT', `✅ 已注册事件: ${eventName}`);
        }
        ...
        if (eventTypes.GENERATION_ENDED || 'generation_ended') {
            ...
            SillyTavern.eventSource.on(eventName, (messageId) => {
                addLog('EVENT', `✅ [GENERATION_ENDED] 触发 - 消息ID: ${messageId}`);
                debouncedProcess();
            });
            addLog('EVENT', `✅ 已注册事件: ${eventName}`);
        }
        ...
        if (eventTypes.MESSAGE_RECEIVED || 'message_received') {
            ...
            SillyTavern.eventSource.on(eventName, (messageId) => {
                addLog('EVENT', `✅ [MESSAGE_RECEIVED] 触发 - 消息ID: ${messageId}`);
                debouncedProcess();
            });
            addLog('EVENT', `✅ 已注册事件: ${eventName}`);
        }

        addLog('EVENT', '✅ 所有酒馆事件监听已注册完成');
    }
})();
